...
%  2. ONLY edit needed: \MEMBERB / \MEMBERC (teammate names). Group=24 and
%     all metrics are already filled from the canonical A100 run.
%  3. (Optional) if you re-run, refresh the "RUN NUMBERS" block from
%     final/results.json — every metric in the body is a macro.
% =====================================================================
...
% --- identity --------------------------------------------------------
%  Only \MEMBERB / \MEMBERC need a human: put your teammates' real names
%  (or delete the two extra contribution bullets if you are a smaller team).
\newcommand{\GROUP}{24}
\newcommand{\MEMBERA}{Taylan İrak}
\newcommand{\MEMBERB}{Teammate 2 (name)}
\newcommand{\MEMBERC}{Teammate 3 (name)}
...
% ---- ensemble (dev) -------------------------------------------------
\newcommand{\EnsAcc}{0.748}\newcommand{\EnsRho}{+0.573}
\newcommand{\EnsIntAcc}{0.733}\newcommand{\EnsIntRho}{+0.560}
\newcommand{\EnsVerdict}{Below OK}              % "OK" / "Good" / "Below OK"
\newcommand{\Wa}{0.00}\newcommand{\Wb}{0.00}\newcommand{\Wc}{1.00} % ensemble.weights
% ---- classification view (results.json -> "classification") ---------
\newcommand{\MacroF}{0.272}
\newcommand{\MacroP}{0.335}
\newcommand{\MacroR}{0.307}
% ---- ablations ------------------------------------------------------
\newcommand{\AblIntRho}{+0.560}\newcommand{\AblContRho}{+0.573}
...
($\ge0.70/\ge0.60$). The calibrated ensemble is \EnsVerdict{}: the
no-worse-than-best-single guard found no blend that beat the best hold-out
component, so it fell back to \textbf{Component~C alone} (weights
A~\Wa{}/B~\Wb{}/C~\Wc{}) --- i.e.\ the submitted system is the
isotonic-recalibrated Likert head.
\textbf{Classification view:} confusion matrix
...
\begin{subfigure}{0.48\textwidth}\includegraphics[width=\linewidth]{fig2_confusion_matrix.png}
  \caption{Confusion matrix (dev, rounded).}\label{fig:cm}\end{subfigure}\hfill
\begin{subfigure}{0.48\textwidth}\includegraphics[width=\linewidth]{fig3_per_class_f1.png}
...
\begin{figure}[H]\centering
\begin{subfigure}{0.48\textwidth}\includegraphics[width=\linewidth]{fig7_training_curves.png}
  \caption{Hold-out $\rho$ per epoch (B/C, 3 seeds).}\label{fig:tc}\end{subfigure}\hfill
\begin{subfigure}{0.48\textwidth}\includegraphics[width=\linewidth]{fig8_ablations.png}
...
\item \emph{Ensemble drop-one} --- since the guard selected C-only
      (A,B weight $0$), removing A or B leaves the system unchanged while
      removing \DropWorst{} collapses it: the blend is exactly C.
...
\begin{figure}[H]\centering
\begin{subfigure}{0.31\textwidth}\includegraphics[width=\linewidth]{fig6_calibration.png}
  \caption{Calibration.}\label{fig:cal}\end{subfigure}\hfill
\begin{subfigure}{0.31\textwidth}\includegraphics[width=\linewidth]{fig9_sota_baseline_comparison.png}
  \caption{System vs.\ baselines.}\label{fig:sota}\end{subfigure}\hfill
\begin{subfigure}{0.31\textwidth}\includegraphics[width=\linewidth]{fig10_uncertainty_vs_error.png}
...
(2)~The ensemble does not beat the best single component on dev
(\EnsRho{} vs.\ B's \Brho{}): the guard correctly fell back to the
strongest \emph{hold-out} component (C), but C is not the strongest on
\emph{dev} (B is) --- an irreducible hold-out/dev mismatch on a 276-row
homonym-disjoint hold-out, not a blending bug. (3)~Continuous predictions
...
\begin{figure}[H]\centering
\begin{subfigure}{0.48\textwidth}\includegraphics[width=\linewidth]{fig1_label_eda.png}
  \caption{Label EDA (train).}\end{subfigure}\hfill
\begin{subfigure}{0.48\textwidth}\includegraphics[width=\linewidth]{fig4_macro_PR.png}
  \caption{Macro precision/recall.}\label{fig:macropr}\end{subfigure}\\[4pt]
\begin{subfigure}{0.48\textwidth}\includegraphics[width=\linewidth]{fig5_pr_curves.png}
  \caption{One-vs-rest precision--recall curves.}\label{fig:prc}\end{subfigure}\hfill
\begin{subfigure}{0.48\textwidth}\includegraphics[width=\linewidth]{fig9_sota_baseline_comparison.png}
